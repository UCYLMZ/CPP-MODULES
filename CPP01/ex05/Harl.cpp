...
void	Harl::complain(std::string level)
{
	...
	funcs			functions[4] = {&Harl::debug, &Harl::info, &Harl::warning, &Harl::error};

	std::string 	*location = std::find(std::begin(levels), std::end(levels), level);
	int index = std::distance(std::begin(levels), location);
